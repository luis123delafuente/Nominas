...
Uso:
    ./venv/bin/python3 scripts/aplicar_actualizacion.py ORIGEN DESTINO [--aplicar]
...
def hacer_backup(destino: Path) -> Path:
...
def main() -> None:
    ...
    for nombre in archivos_sueltos:
        existe = "sí" if (origen / nombre).exists() else "NO EXISTE EN ORIGEN, se omite"
        print(f"  - {nombre}  ({existe})")
    print()
    ...
    print()
    backup_dir = hacer_backup(destino)
